...
- In scope: querying GitHub Projects, retrieving project details, listing project fields and items, updating project item field values, adding issues or pull requests to projects, creating pull requests when explicitly requested, reading issues and pull requests, updating issue or pull request metadata, adding issue comments, and summarizing project status.
...
- The expected issue and pull request tools are: `issue_read`, `issue_write`, `list_issues`, `search_issues`, `add_issue_comment`, `create_pull_request`, `pull_request_read`, `list_pull_requests`, `search_pull_requests`, and `update_pull_request`.
